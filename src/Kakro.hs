...
instance Puzzle Kakro where
  ...
  candidate b (i, j) = intersect vcand hcand
    where vcand = nub (concat vcands) \\ vfixeds
          hcand = nub (concat hcands) \\ hfixeds
...
kakroCalc :: Int -> Int -> [[Kakro]]
kakroCalc s n = [xs | xs <- comb puzzleElm n, s == sum (map (\(K x) -> x) xs)]

kakro :: Board Kakro -> [Board Kakro]
kakro = solve

instance {-# OVERLAPPING #-} Show (Board Kakro) where
  show = showBoard

instance {-# OVERLAPPING #-} Read (Board Kakro) where
...
sampleData :: String
sampleData =
  unlines ["0\\0  17\\0  15\\0  23\\0  35\\0   0\\0   9\\0  15\\0  13\\0  11\\0"
